...
for($y=$yearcurrent;$y>=2022;$y--) 
{
  ...
  {	
  		...
  		$datecurrent = $y."-".$_m."-01";
  		$query = "select count(*) row_count FROM smoad_device_consolidated_stats_log
		...
		if($res = $db->query($query)) { while($row = $res->fetch_assoc()) {	$row_count = $row['row_count']; if($row_count>0) { $enable_link=true; } } }
  		...
  		{ print "<td><a href='content_ztp_dev/content_ztp_dev_fpdf_generation.php?date=".urlencode($datecurrent)."' target='_blank' >$mname</a></td>"; }
  		else { print "<td>-</td>"; }
