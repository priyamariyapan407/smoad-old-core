...
{	$_data = chop($_data);
	...
	if(strpos($_data, 'serialnumber') !== false && strpos($_data, 'api_device_pubkey') !== false)
	{	$lines = explode("&", $_data);
		...
		{  $line = chop($line);
			...
			{	switch($valuepair[0]) 
				{	case 'serialnumber'		: { $_serialnumber_parsed=$valuepair[1];
					...
					case 'api_device_pubkey'	: { $_api_device_pubkey=$valuepair[1]; break; }
				}
			}
		}
	}
...
$query2="update smoad_devices set api_device_pubkey=\"$_api_device_pubkey\" where serialnumber=\"$_serialnumber\"";
$db->query($query2);
