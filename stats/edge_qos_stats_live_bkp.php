...
<?php
   ...
   // echo $where_clause_gw_serial_vxlan_id;exit;
    ...
    function get_query_result($columns,$time_interval){
        ...
        return "SELECT log_timestamp,DATE_FORMAT(log_timestamp, '%Y-%m-%d %H:%i:00') AS log_timestamp, $columns FROM smoad_device_network_qos_stats_log  WHERE log_timestamp >= NOW() - $time_interval AND log_timestamp < NOW() AND $glbl_where_clause_gw_serial_vxlan_id GROUP BY DATE_FORMAT(log_timestamp, '%Y-%m-%d %H'), MINUTE(log_timestamp),http_rx_pkts,https_rx_pkts,iperf_rx_pkts,zoom_rx_pkts,microsoft_teams_rx_pkts,icmp_rx_pkts,tcp_rx_pkts,udp_rx_pkts, DIV 10  ORDER BY log_timestamp";
...
<!-- Graph placeholders -->
       ...
       $current_time_qry = get_query_result("sum(http_rx_bytes) as http_rx_bytes, sum(https_rx_bytes) as https_rx_bytes, sum(iperf_rx_bytes) as iperf_rx_bytes, sum(zoom_rx_bytes) as zoom_rx_bytes, sum(microsoft_teams_rx_bytes) as microsoft_teams_rx_bytes,sum(skype_rx_bytes) as skype_rx_bytes, sum(voip_rx_bytes) as voip_rx_bytes, sum(other_rx_bytes) as other_rx_bytes,sum(icmp_rx_bytes) as icmp_rx_bytes,sum(tcp_rx_bytes) as tcp_rx_bytes,sum(udp_rx_bytes) as udp_rx_bytes","INTERVAL 24 HOUR");
        ...
        {
            array_push($rx_bytes_labl,$row2['log_timestamp']);
        ...
        while($row2 = $res1->fetch_assoc())
        {
            array_push($wk_rx_bytes_labl,$row2['log_timestamp']);
        ...
        while($row2 = $res1->fetch_assoc())
        {
            array_push($rx_pkt_labl,$row2['log_timestamp']);
        ...
        while($row2 = $res1->fetch_assoc())
        {
            array_push($wk_rx_pkt_labl,$row2['log_timestamp']);
        ...
        while($row2 = $res1->fetch_assoc())
        {
            array_push($tx_byte_labl,$row2['log_timestamp']);
        ...
        while($row2 = $res1->fetch_assoc())
        {
            array_push($wk_tx_byte_labl,$row2['log_timestamp']);
       ...
       $current_time_qry = get_query_result("sum(http_tx_pkts) as http_tx_pkts,sum(https_tx_pkts) as https_tx_pkts,sum(iperf_tx_pkts) as iperf_tx_pkts,sum(zoom_tx_pkts) as zoom_tx_pkts,sum(microsoft_teams_tx_pkts) as microsoft_teams_tx_pkts,sum(skype_tx_pkts) as skype_tx_pkts,sum(voip_tx_pkts) as voip_tx_pkts,sum(other_tx_pkts) as other_tx_pkts,sum(icmp_tx_pkts) as icmp_tx_pkts,sum(tcp_tx_pkts) as tcp_tx_pkts,sum(udp_tx_pkts) as udp_tx_pkts","INTERVAL 24 HOUR");
        ...
        while($row2 = $res1->fetch_assoc())
        {
            array_push($tx_pkt_labl,$row2['log_timestamp']);
        ...
        // start of week tx pkt chart data's with 10 minutes back
            ...
            while($row2 = $res1->fetch_assoc())
            {
                array_push($wk_tx_pkt_labl,$row2['log_timestamp']);
